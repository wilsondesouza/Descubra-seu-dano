...
# Limpar Tela #
    ...
    class ArmaInvalidaException(Exception):
        if self.personagem_jogado == 'Slingshot' and calculadora.personagem_jogado != 'Walter':
            s(1)
...
# Usuário informará seu personagem, o programa realizará a busca no dicionário "multiplicador" e passará para a próxima parte
    def personagem(self, dicionario):
        #personagensOrdenadss = sorted(dicionario.keys())
        #print("Personagens disponíveis:")
        #for personagem in personagensOrdenadss:
        #    s(.5)
        #    print(personagem)
        seguir = True
        while seguir:
            try:
                ...
                multiplicador = dicionario[self.personagem_jogado]
            except KeyError:
...
# O usuário informará sua arma, o programa buscará no dicionário "weapon" e passará para a próxima parte
    def arma(self, dicionario):
        #armasOrdenadas = sorted(dicionario.keys())
        #print("Armas disponíveis:")
        #for arma in armasOrdenadas:
        #    s(.5)
        #    print(arma)
        seguir = True
        while seguir:
            try:
                ...
                weapon = dicionario[self.arma_usada]
            except KeyError:
                print("Coloque uma arma jogável")
                tentativa = input("Quer tentar de novo?\n").replace(" ", "").title()
                if tentativa != "Sim":
                    print("Ok. Tchau")
                    s(1)
            ...
            except ArmaInvalidaException:
                    print(f'A arma {self.arma_usada} não é adequada para {self.personagem_jogado}') 
                    break
            else:
                s(.5)
        ...
        for key in Dicionário.multiplicador.keys():
            if key.startswith(self.personagem_jogado):
                ...
                if self.personagem_jogado == 'Wolfgang':
                    variacao = input('Wolfão foi pra academia?"S/N"''\n').replace(' ', '').title()
                    s(1)
                    match variacao:
                        case "N":
